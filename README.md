...
漫画コラボ系のTシャツを閲覧していたが、サイト毎にコラボしていることが多く、毎回複数サイトを開いて閲覧するのが面倒くさかったため、もしそれらを定期的に収集＆まとめて表示するサイトがあれば便利だと思い、試しに作成。   
webサイトとしての公開の予定はありません。   
ローカルのDockerで動かすことしか想定してないローカルアプリケーションです。

アプリ機能「商品収集、保存、Tシャツ商品表示」、Redis、DB、Docker、GitHub Actionsを実装、構築済みです。
...
- **対象サイト**：構造固定のアニメグッズ販売サイト複数。サイトはアプリ側で固定。
- **特徴**：バッチにて設定された時間に指定のサイトからHTMLを収集し、商品情報解析・取得・保存。フロントエンドからの保存データ取得をNext.jsのAPI Routesを用いて実装。
...
商品カード一覧(画像、商品名、値段、元サイトへのリンク)、ロードアイコン、エラーメッセージ表示領域
